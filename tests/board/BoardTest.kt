...
internal class BoardTest {
	...
	fun testGetMoveValue() {
		...
		val moves=listOf(1 to 2,1 to 3,2 to 0,4 to 0,5 to 2,6 to 2,6 to 3)
		val results=listOf(1,1,3,3,1,1,2)

		for((i,move) in moves.withIndex()) {
			val (row,column)=move
			val cell=Board.getCellNumber(row,column)
			assertEquals(results[i],board.getMoveValue(cell),"Wrong move value for cell ($row,$column)")
		}
	}

	@Test
	fun testGetEmptyFields() {
		val expected=60
		val actual=startingBoard.emptyFields
		assertEquals(expected,actual)
	}
}